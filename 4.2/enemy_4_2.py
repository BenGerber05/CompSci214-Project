...
class Functions: #Methods to be used on an array of enemies    
    ...
    ref_left = Enemy.HOR_START
    ref_right = Enemy.HOR_SPACING*(Enemy.COLUMNS-1) + Enemy.HOR_START      
    ...
    def move(self,enemy_list:list[list], spawn_chance: float, speed: float, last_fire: list[float], enemy_projectiles: list[Projectile]): 
        ...
        Functions.ref_left += Enemy.ref_direction*speed
        Functions.ref_right += Enemy.ref_direction*speed

        if Functions.ref_left <= Enemy.LEFT_BOUND and Enemy.ref_direction<0: #if an imaginary enemy in the 0th column hits the left bound => switch direction and spawn a row
                    self.drop(enemy_list)
                    enemy_list += [self.spawnRow(spawn_chance, last_fire)] 
                 
        if Functions.ref_right >= Enemy.RIGHT_BOUND and Enemy.ref_direction>0: #if an imaginary enemy in the maximum column hits the right bound => switch direction
                    self.drop(enemy_list)
